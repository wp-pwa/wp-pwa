...
describe('Core › Build', () => {
  ...
  test('channel amp', () => {
    const { build } = Stores.create({ build: { ...defaultProps, channel: 'amp' } });
  ...
  test('device mobile', () => {
    const { build } = Stores.create({ build: { ...defaultProps, device: 'mobile' } });
  ...
  test('device tablet', () => {
    const { build } = Stores.create({ build: { ...defaultProps, device: 'tablet' } });
  ...
  test('device desktop', () => {
    const { build } = Stores.create({ build: { ...defaultProps, device: 'desktop' } });
    expect(build.isMobile).toBe(false);
    expect(build.isTablet).toBe(false);
  ...
  test('rendering ssr', () => {
    const { build } = Stores.create({ build: { ...defaultProps, rendering: 'ssr' } });
    expect(build.isSsr).toBe(true);
    expect(build.isCsr).toBe(false);
    expect(build.isRendering('ssr')).toBe(true);
  });
  test('rendering csr', () => {
    const { build } = Stores.create({ build: { ...defaultProps, rendering: 'csr' } });
  ...
  test('no initialUrl', () => {
    ...
    expect(build.initialUrl).toBe(null);
  });
